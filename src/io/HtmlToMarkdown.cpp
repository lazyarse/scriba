            ...
            [ctx, page, baseLiteral](const QVariant &) {
                page->runJavaScript(
                    QStringLiteral(
                        ...
                        "      var md = td.turndown(child).trim();"
                        "      if (!md) continue;"
                        "      if (tag === 'dt') {"
                        "        if (cur && cur.defs.length) cur = null;"
                        "        if (!cur) { cur = { terms: [], defs: [] }; groups.push(cur); }"
                        "        cur.terms.push(md);"
                        "      } else {"
                        "        if (!cur) { cur = { terms: [], defs: [] }; groups.push(cur); }"
                        "        cur.defs.push(md);"
                        ...
                        "      var g = groups[i];"
                        "      var block = g.terms.join('\\n');"
                        "      if (g.defs.length)"
                        "        block += '\\n' + g.defs.map(function (d) { return ': ' + d; }).join('\\n');"
                        "      out.push(block);"
                        "    }"
                        "    return '\\n\\n' + out.join('\\n\\n') + '\\n\\n';"
                        "  }"
                        ...
                        "    /* #WxH size suffix (Scriba syntax, MdRenderer::parseDimensions): read"
                        "       explicit sizes from inline style (Scriba's export writes exact"
                        "       width/height for SVG and max-width/max-height caps for raster) or"
                        "       the width/height attributes (DOCX imports); integer px only — the"
                        "       suffix cannot express fractions or percentages. addRule prepends,"
                        "       so this shadows the built-in image rule. */"
                        "    var size = '';"
                        "    var pxRe = /^(\\d+)(px)?$/;"
                        "    var wm = (node.style.width || node.style.maxWidth || node.getAttribute('width') || '').match(pxRe);"
                        "    var hm = (node.style.height || node.style.maxHeight || node.getAttribute('height') || '').match(pxRe);"
                        "    if (wm && hm) size = '#' + wm[1] + 'x' + hm[1];"
                        "    else if (wm) size = '#' + wm[1] + 'x';"
                        "    else if (hm) size = '#x' + hm[1];"
                        "    return '![' + alt + '](' + src + size"
                        "         + (title ? ' \"' + title.replace(/\"/g, '\\\\\"') + '\"' : '') + ')';"
                        "  }"
                        "});"
                        "var md = td.turndown(root);"
                        "/* Footnote markers ([^..]) are intentional Scriba"
                        " syntax; undo turndown's bracket escaping. */"
