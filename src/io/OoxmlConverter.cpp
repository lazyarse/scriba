...
void Converter::parseRelationships()
{
...
bool Converter::toggleVal(QXmlStreamReader &r)
{
...
bool Converter::listOrdered(const QString &numId, int level) const
{
...
QString Converter::plainParagraph(QXmlStreamReader &r)   // at <w:p>, consumes it
{
    QString out;
    while (r.readNextStartElement()) {
        const QString lm = r.name().toString();
...
QString Converter::plainPart(const QByteArray &bytes)
{
...
void Converter::gatherHeadersFooters()
{
...
OoxmlToHtmlResult Converter::run(const QByteArray &doc)
{
...
void Converter::emitBlocks(QXmlStreamReader &r, QStringList &frags, ListState &st)
{
    while (r.readNextStartElement()) {
        const QString lm = r.name().toString();
...
Para Converter::emitParagraph(QXmlStreamReader &r)
{
    Para para;
    while (r.readNextStartElement()) {
        const QString lm = r.name().toString();
...
void Converter::parseParaProps(QXmlStreamReader &r, Para &para)
{
    QString numId;
    while (r.readNextStartElement()) {
        const QString lm = r.name().toString();
...
void Converter::emitRun(QXmlStreamReader &r, QString &content)
{
    RunProps props;
    QString frag;
    while (r.readNextStartElement()) {
...
void Converter::parseRunProps(QXmlStreamReader &r, RunProps &props)
{
    while (r.readNextStartElement()) {
        const QString lm = r.name().toString();
...
QString Converter::applyFormatting(const RunProps &props, const QString &content) const
{
    QString c = content;
...
void Converter::emitHyperlink(QXmlStreamReader &r, QString &content)
{
    QString href;
...
QString Converter::image(QXmlStreamReader &r)
{
    QString rId;
    int cx = 0, cy = 0;
    int depth = 1;
...
QString Converter::mathLatex(QXmlStreamReader &r)
{
...
void Converter::warn(const QString &msg)
{
...
void Converter::closeLists(QStringList &frags, ListState &st)
{
...
void Converter::addPara(QStringList &frags, ListState &st, const Para &para)
{
    if (para.isListItem()) {
...
void Converter::emitTable(QXmlStreamReader &r, QStringList &frags)
{
    struct Row {
        QVector<Cell> cells;
        bool header = false;
...
Cell Converter::parseCell(QXmlStreamReader &r)
{
    Cell c;
    QStringList cells;
    ListState st;
...
OoxmlToHtmlResult Converter::finalize(const QStringList &frags)
{
